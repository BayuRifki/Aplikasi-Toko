...
public class AplikasiToko extends JFrame{
 ...
 final static String StringConnection ="jdbc:mysql://localhost:3306/dbtokobanajaya?user=root";
 ...
 private String DaftarSubMenu[] = {"Barang","Customer","User Account",
"Penjualan"};
 ...
 static String JudulKolomTabelDaftarBarang[] = {"Kode","Nama Barang",
"Satuan"};
 static DefaultTableModel ModelTabelDaftarBarang = new
DefaultTableModel(null,JudulKolomTabelDaftarBarang);
 ...
 static String JudulKolomTabelDaftarCustomer[] = {"Kode","NamaCustomer","Alamat Customer"};
 static DefaultTableModel ModelTabelDaftarCustomer = new
DefaultTableModel(null,JudulKolomTabelDaftarCustomer);
 ...
 static String JudulKolomTabelJualDaftarBarang[] = {"Kode","NamaBarang","Satuan"};
 static DefaultTableModel ModelTabelJualDaftarBarang = new
DefaultTableModel(null,JudulKolomTabelJualDaftarBarang);
 ...
 static String JudulKolomTabelJualDaftarCustomer[] = {"Kode","NamaCustomer","Alamat Customer"};
 static DefaultTableModel ModelTabelJualDaftarCustomer = new
DefaultTableModel(null,JudulKolomTabelJualDaftarCustomer);
 ...
 private static JButton TblJualDaftarCustomerPilih = new
JButton("Pilih");
 private static JButton TblJualDaftarCustomerBatal = new
JButton("Batal");
 ...
 static String UserAccountTabelJudulKolom[] = {"Menu","Sub Menu","HakAkses"};
 ...
 static String JudulKolomTabelPenjualan[] = {"No.","Kode","Nama Barang",
"Harga","Jumlah","Total"};
 static DefaultTableModel ModelTabelPenjualan = new
DefaultTableModel(null,JudulKolomTabelPenjualan);
...
Connection cn = null;
 ...
 JDBC_Err=true;
 JOptionPane.showMessageDialog(null,"Koneksi ke database banajaya gagal\n"+ex,"Kesalahan",JOptionPane.ERROR_MESSAGE);
 }
 ...
 JDBC_Err=true;
 JOptionPane.showMessageDialog(null,"Koneksi ke database dbtokobanajaya gagal\n"+ex,"Kesalahan",JOptionPane.ERROR_MESSAGE);
 } 
if (!JDBC_Err){
    try {
 cn = DriverManager.getConnection(StringConnection);
...
TxtAlamatCustomer.setText(rset.getString("AlamatCustomer"));
 ...
 JDBC_Err=true;
 JOptionPane.showMessageDialog(null,"Koneksi ke database tokobanajaya gagal\n"+ex,"Kesalahan",JOptionPane.ERROR_MESSAGE);
 }
if (!JDBC_Err){
 try { 
     cn = DriverManager.getConnection(StringConnection);
     String SQLStatemen = "Select TbDetilPenjualan.NoNota,TbPenjualan.KodeCustomer,TbCustomer.NamaCustomer"+"\"TbDetilPenjualan.KodeBarang,TbBarang.NamaBarang,TbDetilPenjualan.Harga, TbDetilPenjualan.Qty"+"(TbDetilPenjualan.Harga*TbDetilPenjualan.Qty) as Subtotal from TbDetilPenjualan inner join"+"TbPenjualan on TbDetilPenjualan.NoNota=TbPenjualan.NoNota inner join TbCustomer on"+"TbPenjualan.KodeCustomer=TbCustomer.KodeCustomer innerjoin TbBarang on"+"TbDetilPenjualan.KodeBarang=TbBarang.KodeBarang where TbDetilPenjualan.NoNota="+"TxtNoNotaJual.getText()";
     Statement sta = cn.createStatement();
 ResultSet rset = sta.executeQuery(SQLStatemen);
...
TxtKembaliJual.setText(Integer.toString(Integer.parseInt(TxtBayarJual.getText())-Total));
 ...
 frmDaftarBarang.setLocation(dimensi.width/2-
frmDaftarBarang.getWidth()/2,dimensi.height/2-
frmDaftarBarang.getHeight()/2);
 ...
 frmDaftarCustomer.setLocation(dimensi.width/2-
frmDaftarCustomer.getWidth()/2,dimensi.height/2-
frmDaftarCustomer.getHeight()/2);
...
frmJualDaftarBarang.getHeight()/2);
 ...
 frmJualDaftarCustomer.setLocation(dimensi.width/2-
frmJualDaftarCustomer.getWidth()/2,dimensi.height/2-
frmJualDaftarCustomer.getHeight()/2);
 ...
 // set Column width
 TableColumn ColumnTabelDaftarBarang =
TabelDaftarBarang.getColumnModel().getColumn(0);
 ColumnTabelDaftarBarang.setPreferredWidth(80);
 ColumnTabelDaftarBarang =
TabelDaftarBarang.getColumnModel().getColumn(1);
 ColumnTabelDaftarBarang.setPreferredWidth(230);
 ColumnTabelDaftarBarang =
TabelDaftarBarang.getColumnModel().getColumn(2);
 ColumnTabelDaftarBarang.setPreferredWidth(100);

 //Header font set to Bold
 TabelDaftarBarang.getTableHeader().setFont( new Font( "Dialog" ,
Font.BOLD, 13 ));
...
TblDaftarBarangBatal.setBounds((frmDaftarBarang.getWidth()/2)+20,frmDaftarBarang.getHeight()-85,80,30);
 ...
 // set Column width
 TableColumn ColumnTabelDaftarCustomer =
TabelDaftarCustomer.getColumnModel().getColumn(0);
 ColumnTabelDaftarCustomer.setPreferredWidth(80);
 ColumnTabelDaftarCustomer =
TabelDaftarCustomer.getColumnModel().getColumn(1);
 ColumnTabelDaftarCustomer.setPreferredWidth(230);
 ColumnTabelDaftarCustomer =
TabelDaftarCustomer.getColumnModel().getColumn(2);
...
TblDaftarCustomerBatal.setBounds((frmDaftarCustomer.getWidth()/2)+20,frmDaftarCustomer.getHeight()-85,80,30);
 ...
 // set Column width
 TableColumn ColumnTabelJualDaftarBarang =
TabelJualDaftarBarang.getColumnModel().getColumn(0);
 ColumnTabelJualDaftarBarang.setPreferredWidth(80);
 ColumnTabelJualDaftarBarang =
TabelJualDaftarBarang.getColumnModel().getColumn(1);
 ColumnTabelJualDaftarBarang.setPreferredWidth(230);
 ColumnTabelJualDaftarBarang =
TabelJualDaftarBarang.getColumnModel().getColumn(2);
 ColumnTabelJualDaftarBarang.setPreferredWidth(100);

 //Header font set to Bold
 TabelJualDaftarBarang.getTableHeader().setFont( new Font( "Dialog" ,
Font.BOLD, 13 ));
...
TabelJualDaftarCustomer.getColumnModel().getColumn(2);
 ...
 //Header font set to Bold
 TabelJualDaftarCustomer.getTableHeader().setFont( new Font( "Dialog" ,
Font.BOLD, 13 ));
...
UserAccountTabel.getColumnModel().getColumn(2);
 ...
 for (i=0;i<=3;i++){
 UserAccountModelTabel.insertRow(i,new
Object[]{DaftarMenu[i/3],DaftarSubMenu[i],"Deny"});
 }
...
pnlCustomerJual.setBorder(BorderFactory.createTitledBorder("Customer"));
 ...
 pnlDetilPenjualan.setBorder(BorderFactory.createTitledBorder("DetilPenjualan"));
 ...
 // set Column width
 TableColumn ColumnTabelPenjualan =
TabelPenjualan.getColumnModel().getColumn(0);
 ...
 //Header font set to Bold
 TabelPenjualan.getTableHeader().setFont( new Font( "Dialog" ,
Font.BOLD, 13 ));
...
ModelTabelPenjualan.removeRow(TabelPenjualan.getSelectedRow());
 ...
 public void mousePressed(MouseEvent me){
 if(me.isPopupTrigger()){
 MenuPopupPenjualan.show(me.getComponent(), me.getX(),
me.getY());
 }
 }
 public void mouseReleased(MouseEvent Me){
...
UserAccountTabel.setValueAt("Deny",UserAccountTabel.getSelectedRow(),2);
 ...
 /*class untuk memproses ActionEvent dari tombol di form Daftar Barang di
form Barang*/
...
/*class untuk memproses ActionEvent dari tombol di form Daftar Customer
di form Customer*/
...
rset.getString("NamaCustomer"),rset.getString("AlamatCustomer")});
 ...
 SQLStatemen = "delete from TbCustomer where KodeCustomer='"
...
JOptionPane.QUESTION_MESSAGE,null,arrOpsi,arrOpsi[0]);
 if (pilih==0){
 SQLStatemen = "update TbCustomer setNamaCustomer='"+TxtNamaCustomer.getText()+
...
(((String)UserAccountTabel.getModel().getValueAt(i,2)).equals("View")) {
 ...
 bufHakAkses =
bufHakAkses.concat(Character.toString((char)Math.round(Math.random()*255)))
;
 }
...
TxtIDUserUserAccount.getText()+"'";
 ...
 int TombolOpsi=JOptionPane.showConfirmDialog(null,
PasswordField, "Ketik Password", JOptionPane.OK_CANCEL_OPTION);
 if (TombolOpsi==JOptionPane.OK_OPTION){
 char[] Password = PasswordField.getPassword();
 PasswordField = new JPasswordField();
 TombolOpsi=JOptionPane.showConfirmDialog(null,
PasswordField, "Ketik Ulang Password", JOptionPane.OK_CANCEL_OPTION);
 if (TombolOpsi==JOptionPane.OK_OPTION){
...
"Ketik Password Lama", JOptionPane.OK_CANCEL_OPTION);
 ...
 JOptionPane.showMessageDialog(null,"Kesalahan pada hash SHA256\n"+ex,"Kesalahan",JOptionPane.ERROR_MESSAGE);
 }
 md.update(inputPassword.getBytes());
 byte bytePassword[] = md.digest();
...
16).substring(1));
 ...
 PasswordField = new JPasswordField();
 TombolOpsi=JOptionPane.showConfirmDialog(null,
PasswordField, "Ketik Password Baru", JOptionPane.OK_CANCEL_OPTION);
 if (TombolOpsi==JOptionPane.OK_OPTION){
 Password = PasswordField.getPassword();
 PasswordField = new JPasswordField();
 TombolOpsi=JOptionPane.showConfirmDialog(null,
PasswordField, "Ketik Ulang Password Baru", JOptionPane.OK_CANCEL_OPTION);
 if (TombolOpsi==JOptionPane.OK_OPTION){
 char[] RetypePassword = PasswordField.getPassword();
...
ModelTabelJualDaftarCustomer.insertRow(ModelTabelJualDaftarCustomer.getRowCount(),new
...
rset.getString("NamaBarang"),rset.getString("SatuanBarang")});
 ...
 FormatWaktu = new SimpleDateFormat ("HH:mm:ss EEEE, dd-MMyyyy",Locale.getDefault());
 ...
 if (rset.getRow()>0){
 String NoBaru=
Integer.toString(Integer.parseInt(rset.getString("NoNota").substring(6,10))+1);
 ...
 if
(TxtKodeBarangJual.getText().trim().equals(((String)TabelPenjualan.getModel
().getValueAt(i,1)).trim())){
 ada=true;
 } else {
 i++;
...
SubTtl=Integer.parseInt((String)TabelPenjualan.getModel().getValueAt(i,5));
 ...
 java.util.Date WaktuSekarang;
 SimpleDateFormat FormatWaktu;
 FormatWaktu = new SimpleDateFormat ("yyyy/MM/ddHH:mm:ss",Locale.getDefault());
 WaktuSekarang = new java.util.Date(); 
String Tanggal="";
 ...
 sta.close();
 rset.close();
 SQLStatemen = "insert into TbPenjualan values('"+TxtNoNotaJual.getText()+"','"+Tanggal+"','"+TxtKodeCustomerJual.getText
()+"','"+"B"+"')";
 sta = cn.createStatement();
 simpan = sta.executeUpdate(SQLStatemen);
 sta.close();
...
Integer.parseInt((String)TabelPenjualan.getModel().getValueAt(i,3))
 + "', Qty='"+
Integer.parseInt((String)TabelPenjualan.getModel().getValueAt(i,4))+ "' where NoNota='"+TxtNoNotaJual.getText()+"'and KodeBarang='"+(String)TabelPenjualan.getModel().getValueAt(i,1)+"'";
 sta = cn.createStatement();
 simpan = sta.executeUpdate(SQLStatemen);
 sta.close();
 } else {
